...
namespace  // Concrete FSM implementation
{
    ...
    {
        template <class T>
        void visit_state(T* astate, int i, int j)
        {
            std::cout << "visiting state:" << typeid(*astate).name()
                << " with data:" << i << ' ' << j << std::endl;
        }
    };
    ...
    struct player_ : public msm::front::state_machine_def<player_,my_visitable_state>
    {
        ...
        struct Empty : public msm::front::state<my_visitable_state>
        {
            ...
            void accept(SomeVisitor& vis,int i, int j) const
            {
                vis.visit_state(this,i, j);
            }
        };
    ...
    void test()
    {
        ...
        SomeVisitor vis;
        p.visit_current_states(boost::ref(vis),1,2);
        p.process_event(open_close()); pstate(p);
        // visiting Empty and AllOk, but only Empty cares
        p.visit_current_states(boost::ref(vis),2,3);
        ...
        p.visit_current_states(boost::ref(vis),3,4);
        ...
        p.visit_current_states(boost::ref(vis),5,6);
