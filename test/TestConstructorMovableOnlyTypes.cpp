...
struct bistable_switch_ : public msm::front::state_machine_def<bistable_switch_> {
    bistable_switch_(std::unique_ptr<Lightbulp> bulp, int load) : bulp_(std::move(bulp))
    {
    ...
    void turn_on(ev_toggle const&)
    {
        bulp_->current = 11;
        std::cout << "bistable_switch::turn_on" << std::endl;
    }
    void turn_off(ev_toggle const&)
    {
        bulp_->current = 9;
        std::cout << "bistable_switch::turn_off" << std::endl;
    }
