...
typedef boost::parameter::parameters<boost::parameter::required<boost::msm::back::tag::front_end>,
    boost::parameter::optional<boost::parameter::deduced<boost::msm::back::tag::history_policy>,
                                         has_history_policy<boost::mpl::_>>,
    boost::parameter::optional<boost::parameter::deduced<boost::msm::back::tag::compile_policy>,
                                         has_compile_policy<boost::mpl::_>>,
    boost::parameter::optional<boost::parameter::deduced<boost::msm::back::tag::fsm_check_policy>,
                                         has_fsm_check<boost::mpl::_>>,
    boost::parameter::optional<boost::parameter::deduced<boost::msm::back::tag::queue_container_policy>,
                                         has_queue_container_policy<boost::mpl::_>>> state_machine_signature;
...
struct make_euml_terminal<T, F, std::enable_if_t<not has_using_declared_table<F>::type::value>>
{
};

template <class T, class F>
struct make_euml_terminal<T, F, std::enable_if_t<has_using_declared_table<F>::type::value>>
    : public proto::extends<typename proto::terminal<boost::msm::state_tag>::type, T, boost::msm::state_domain>
{
};

// library-containing class for state machines.  Pass the actual FSM class as the Concrete parameter.
...
class state_machine
    :  // public Derived
       public boost::parameter::binding<typename state_machine_signature::bind<A0, A1, A2, A3, A4>::type,
           boost::msm::back::tag::front_end>::type,
       public make_euml_terminal<state_machine<A0, A1, A2, A3, A4>,
           typename boost::parameter::binding<typename state_machine_signature::bind<A0, A1, A2, A3, A4>::type,
                                     boost::msm::back::tag::front_end>::type>
{
public:
    // Create ArgumentPack
    typedef typename state_machine_signature::bind<A0, A1, A2, A3, A4>::type state_machine_args;

    // Extract first logical parameter.
    typedef typename boost::parameter::binding<state_machine_args, boost::msm::back::tag::front_end>::type Derived;

    typedef
        typename boost::parameter::binding<state_machine_args, boost::msm::back::tag::history_policy, NoHistory>::type
            HistoryPolicy;

    typedef typename boost::parameter::binding<state_machine_args,
...
private:
    typedef boost::msm::back::state_machine<A0, A1, A2, A3, A4> library_sm;

    typedef std::function<execute_return()> transition_fct;
    typedef std::function<execute_return()> deferred_fct;
    typedef typename QueueContainerPolicy::template In<std::pair<deferred_fct, char>>::type deferred_events_queue_t;
    typedef typename QueueContainerPolicy::template In<transition_fct>::type events_queue_t;

    typedef typename boost::mpl::eval_if<typename is_active_state_switch_policy<Derived>::type,
    ...
    struct visitor_fct_helper<StateType, std::enable_if_t<has_accept_sig<StateType>::value>>
    {
    public:
        ...
        void execute(int index, Args&&... args)
        {
            m_state_visitors[index](std::forward<Args>(args)...);
        }
    ...
    struct deferred_msg_queue_helper<StateType,
        std::enable_if_t<boost::msm::back::has_fsm_deferred_events<StateType>::type::value, int>>
    {
    public:
...
public:
    ...
    typedef typename boost::mpl::eval_if<typename has_initial_event<Derived>::type,
    ...
    typedef typename boost::mpl::eval_if<typename has_final_event<Derived>::type,
    ...
    struct exit_pt : public ExitPoint
    {
        ...
        typedef std::function<execute_return(Event const&)> forwarding_function;
        ...
        void forward_event(ForwardEvent const& incomingEvent)
        {
            ...
            ForwardHelper<std::is_convertible<ForwardEvent, Event>::value>::helper(incomingEvent, m_forward);
        }
        void set_forward_fct(std::function<execute_return(Event const&)> fct)
        {
            m_forward = fct;
        }
        exit_pt() : m_forward()
    ...
    struct row_
    {
        ...
        // if the source is an exit pseudo state, then current_state_type becomes the result of get_owner meaning the
        // containing SM from which the exit occurs
        typedef typename boost::mpl::eval_if<typename has_pseudo_exit<T1>::type,
        ...
        typedef typename boost::mpl::eval_if<typename boost::mpl::is_sequence<T2>::type,
            get_fork_owner<T2, library_sm>,
            boost::mpl::eval_if<typename has_no_automatic_create<T2>::type,
        ...
        static HandledEnum execute(library_sm& fsm, int region_index, int state, transition_event const& evt)
        {
            ...
            if (has_pseudo_exit<T1>::type::value && !is_exit_state_active<T1, get_owner<T1, library_sm>>(fsm))
            {
            ...
            HandledEnum res = ROW::action_call(fsm,
                evt,
                boost::fusion::at_key<current_state_type>(fsm.m_substate_list),
                boost::fusion::at_key<next_state_type>(fsm.m_substate_list),
                fsm.m_substate_list);
    ...
    template <typename ROW, typename StateType>
    struct internal_
    {
    ...
    struct internal_<ROW, library_sm>
    {
        ...
        static HandledEnum execute(library_sm& fsm, int, int, transition_event const& evt)
        {
            ...
            // then call the action method
            HandledEnum res = ROW::action_call(fsm, evt, fsm, fsm, fsm.m_substate_list);
            return res;
        }
    };
    ...
    struct create_real_stt
    {
        ...
        typedef typename boost::mpl::fold<stt_simulated,
            mpl::vector0<>,
    ...
    struct add_forwarding_row_helper
    {
        ...
        typedef typename boost::mpl::fold<all_events,
            Intermediate,
            boost::mpl::push_back<boost::mpl::placeholders::_1, frow<StateType, boost::mpl::placeholders::_2>>>::type
            type;
    };
    ...
    struct get_internal_transition_table
    {
        ...
        typedef typename boost::mpl::fold<recursive_istt,
            boost::mpl::vector0<>,
            boost::mpl::push_back<boost::mpl::placeholders::_1,
                                              make_row_tag<boost::mpl::placeholders::_2, StateType>>>::type
            recursive_istt_with_tag;

        typedef typename boost::mpl::insert_range<original_table,
            typename boost::mpl::end<original_table>::type,
            recursive_istt_with_tag>::type table_with_all_events;

        // and add for every event a forwarding row
        typedef typename boost::mpl::eval_if<typename CompilePolicy::add_forwarding_rows,
    ...
    typedef typename boost::msm::back::generate_event_set<typename create_real_stt<library_sm,
        typename library_sm::internal_transition_table>::type>::type processable_events_internal_table;
    ...
    struct extend_table
    {
        ...
        // Note: these are added first because they must have a lesser prio than the deeper transitions in the sub
        // regions table made of a stt + internal transitions of composite
        typedef
            typename boost::mpl::fold<typename Composite::internal_transition_table,
                boost::mpl::vector0<>,
                boost::mpl::push_back<boost::mpl::placeholders::_1,
                                          make_row_tag<boost::mpl::placeholders::_2, Composite>>>::type internal_stt;

        typedef typename boost::mpl::insert_range<Stt, typename boost::mpl::end<Stt>::type, internal_stt
            // typename get_internal_transition_table<Composite, boost::mpl::true_>::type
            >::type stt_plus_internal;

        // for every state, add its transition table (if any) transformed as frow
        typedef typename boost::mpl::fold<state_list,
            stt_plus_internal,
            boost::mpl::insert_range<boost::mpl::placeholders::_1,
                                              boost::mpl::end<boost::mpl::placeholders::_1>,
                                              get_internal_transition_table<boost::mpl::placeholders::_2,
    ...
    {
        execute_return (library_sm::*pf)(EventType const&, EventSource) = &library_sm::process_event_internal;

        m_events_queue.m_events_queue.push_back(std::bind(pf, this, evt, EVENT_SOURCE_MSG_QUEUE));
    }
    template <class EventType>
    ...
    // Be careful if you do this during event processing, the event will be processed immediately and not kept in the
    // queue
    template <class EventType>
    ...
    struct serialize_state
    {
        ...
        template <typename T>
        std::enable_if_t<has_do_serialize<T>::type::value or is_composite_state<T>::value, void> operator()(T& t) const
        {
            ar_& t;
        }
        template <typename T>
        std::enable_if_t<not(has_do_serialize<T>::type::value or is_composite_state<T>::value), void> operator()(
            T&) const
        {
    ...
    BaseState* get_state_by_id(int id)
    {
        ...
        boost::mpl::for_each<state_list, boost::msm::wrap<boost::mpl::placeholders::_1>>(
            get_state_id_helper(id, &result_state, this));
    ...
    std::enable_if_t<std::is_pointer<State>::value, State> get_state(boost::msm::back::dummy<0> = 0) const
    {
        return const_cast<State>(
            &(boost::fusion::at_key<typename std::remove_const<typename std::remove_pointer<State>::type>::type>(
                m_substate_list)));
    }
    // as a reference
    template <class State>
    std::enable_if_t<std::is_reference<State>::value, State> get_state(boost::msm::back::dummy<1> = 0) const
    {
        return const_cast<State>(
            boost::fusion::at_key<typename std::remove_const<typename std::remove_reference<State>::type>::type>(
                m_substate_list));
    }
    // get a state (non const version)
    // as a pointer
    template <class State>
    std::enable_if_t<std::is_pointer<State>::value, State> get_state(boost::msm::back::dummy<0> = 0)
    {
        return &(static_cast<typename std::add_lvalue_reference<typename std::remove_pointer<State>::type>::type>(
            boost::fusion::at_key<typename std::remove_pointer<State>::type>(m_substate_list)));
    }
    // as a reference
    template <class State>
    std::enable_if_t<std::is_reference<State>::value, State> get_state(boost::msm::back::dummy<1> = 0)
    {
        return boost::fusion::at_key<typename std::remove_reference<State>::type>(m_substate_list);
    }
    ...
    // visit the currently active states (if these are defined as visitable by implementing accept)
    template <typename... Args>
    void visit_current_states(Args... args)
    {
        for (int i = 0; i < nr_regions::value; ++i)
        {
            m_visitors.execute(m_states[i], args...);
        }
    }
    ...
    {
        // to call this function, you need either a state with a deferred_events typedef or that the fsm provides the
        // activate_deferred_events typedef
        BOOST_MPL_ASSERT((has_fsm_deferred_events<library_sm>));
        execute_return (library_sm::*pf)(Event const&, EventSource) = &library_sm::process_event_internal;

        // Deferred events are added with a correlation sequence that helps to identify when an event was added - This
        // is typically to distinguish between events deferred in this processing versus previous.
        m_deferred_events_queue.m_deferred_events_queue.push_back(
            std::make_pair(std::bind(pf, this, e, (EVENT_SOURCE_DIRECT | EVENT_SOURCE_DEFERRED)),
...
public:
    ...
    template <class FirstArg,
        class... Args,
        typename = std::enable_if_t<not boost::proto::is_expr<FirstArg>::type::value>>
    ...
    template <class Expr, class... Args, typename = std::enable_if_t<boost::proto::is_expr<Expr>::value>>
    ...
    state_machine<A0, A1, A2, A3, A4>(library_sm const& rhs) : Derived(rhs)
    {
        if (this != &rhs)
        {
            // initialize our list of states with the ones defined in Derived::initial_state
    ...
    // the following 2 functions handle the terminate/interrupt states handling if one of these states is found, the
    // first one is used
    template <class Event>
    ...
    bool do_pre_msg_queue_helper(EventType const& evt, boost::mpl::false_ const&)
    {
        ...
        if (m_event_processing)
        {
            ...
            m_events_queue.m_events_queue.push_back(
                std::bind(pf, this, evt, EVENT_SOURCE_DIRECT | EVENT_SOURCE_MSG_QUEUE));
    ...
    HandledEnum do_process_helper(EventType const& evt, boost::mpl::false_ const&, bool is_direct_call)
    {
        ...
        // Declaring a local variable here does not hurt and will be "used" to make the code in the handler compilable
        // although the code will never be executed.
        std::exception e;
        BOOST_TRY
        {
            return this->do_process_event(evt, is_direct_call);
    ...
    template <class StateType>
    struct handle_defer_helper<StateType,
        std::enable_if_t<boost::msm::back::has_fsm_deferred_events<StateType>::type::value, int>>
    {
        handle_defer_helper(deferred_msg_queue_helper<library_sm>& a_queue) : m_events_queue(a_queue)
        {
        }
        void do_handle_deferred(bool new_seq = false)
        {
            // A new sequence is typically started upon initial entry to the state, or upon a new transition.  When this
            // occurs we want to process all previously deferred events by incrementing the correlation sequence.
            if (new_seq)
            {
            ...
            // Iteratively process all of the events within the deferred queue upto (but not including) newly deferred
            // events.
            while (!m_events_queue.m_deferred_events_queue.empty())
            {
                typename deferred_events_queue_t::value_type& pair = m_events_queue.m_deferred_events_queue.front();
    ...
    // otherwise
    template <class StateType>
    struct handle_eventless_transitions_helper<StateType,
        std::enable_if_t<boost::msm::back::has_fsm_eventless_transition<StateType>::type::value>>
    {
        ...
        {
            typedef typename boost::mpl::deref<typename boost::mpl::begin<
                typename find_completion_events<StateType>::type>::type>::type first_completion_event;
            if (handled)
            {
                self->process_event_internal(first_completion_event(), source | EVENT_SOURCE_DIRECT);
            }
        }
    ...
    // helper class called in case the event to process has been found in the fsm's internal stt and is therefore
    // processable
    template <class Event>
    ...
    private:
        ...
        // version doing nothing if the event is not in the internal stt and we can save ourselves the time trying to
        // process
    ...
    {
    public:
        region_processing_helper(library_sm* self_, HandledEnum& result_) : self(self_), result(result_)
        {
        }
        template <class Event>
        void process(Event const& evt)
        {
            ...
            HandledEnum res = table::instance.entries[self->m_states[0] + 1](*self, 0, self->m_states[0], evt);
    ...
    // version with visitors
    template <class StateType>
    struct region_processing_helper<StateType,
        std::enable_if_t<boost::mpl::is_sequence<typename StateType::initial_state>::type::value>>
    {
    private:
        ...
        struct In
        {
            ...
            static void process(Event const& evt, library_sm* self_, HandledEnum& result_)
            {
                ...
                typedef dispatch_table<library_sm, complete_table, Event, CompilePolicy> table;
                // +1 because index 0 is reserved for this fsm
                HandledEnum res = table::instance.entries[self_->m_states[region_id::value] + 1](
        ...
        struct In<boost::mpl::int_<nr_regions::value>, Dummy>
        {
            ...
            static void process(Event const& evt, library_sm* self_, HandledEnum& result_)
            {
                // process the event in the internal table of this fsm if the event is processable (present in the
                // table)
    ...
    template <class Event>
    execute_return process_event_internal(Event const& evt, EventSource source = EVENT_SOURCE_DEFAULT)
    {
        ...
        {
            // Process event
            HandledEnum handled = this->do_process_helper<Event>(evt,
            ...
            handle_eventless_transitions_helper<library_sm> eventless_helper(this, (HANDLED_TRUE & handled));
            eventless_helper.process_completion_event(source);

            // After handling, take care of the deferred events, but only if we're not already processing from the
            // deferred queue.
            if (!(EVENT_SOURCE_DEFERRED & source))
            {
                ...
                // Handle any new events generated into the queue, but only if we're not already processing from the
                // message queue.
    ...
    HandledEnum do_process_event(Event const& evt, bool is_direct_call)
    {
        ...
        // if the event has not been handled and we have orthogonal zones, then generate an error on every active state
        // for state machine states contained in other state machines, do not handle but let the containing sm handle
        // the error, unless the event was generated in this fsm (by calling process_event on this fsm object,
        // is_direct_call == true) completion events do not produce an error
...
private:
    ...
    template <typename... Args>
    void composite_accept(Args... args)
    {
        this->accept(args...);
        this->visit_current_states(std::ref(args)...);
    }
    ...
    public:
        init_flags(flag_handler* entries_) : entries(entries_)
        {
        }
        ...
        void operator()(boost::msm::wrap<StateType> const&)
        {
            ...
            {
                // false or forward
                typedef typename boost::mpl::and_<typename is_composite_state<StateType>::type,
                    typename boost::mpl::not_<typename has_non_forwarding_flag<Flag>::type>::type>::type
                    composite_no_forward;
    ...
    template <class State>
    struct create_state_helper<State, std::enable_if_t<State::needs_sm::value>>
    {
    ...
    struct visitor_args
    {
        static constexpr std::size_t N = StateType::accept_sig::args_number;

        template <class State>
        static std::enable_if_t<not is_composite_state<State>::value, void> helper(
            library_sm* sm, int id, StateType& astate)
        {
            sm->m_visitors.insert(id,
                StateType::accept_sig::template bind_accept(
                                      &StateType::accept, astate, std::make_integer_sequence<std::size_t, N>()));
        }

        template <class State>
        static std::enable_if_t<is_composite_state<State>::value, void> helper(
            library_sm* sm, int id, StateType& astate)
        {
            typename StateType::accept_sig::template composite_accept_t<StateType> caccept =
                &StateType::composite_accept;
            sm->m_visitors.insert(id,
                StateType::accept_sig::template bind_accept(
                                      caccept, astate, std::make_integer_sequence<std::size_t, N>()));
        }
    };

// the IBM compiler seems to have problems with nested classes the same seems to apply to the Apple version of gcc 4.0.1
// (just in case we do for < 4.1) and also to MS VC < 8
...
private:
#endif
    // A function object for use with mpl::for_each that stuffs states into the state list.
    template <class ContainingSM>
    struct add_state
    {
        add_state(library_sm* self_, ContainingSM* sm) : self(self_), containing_sm(sm)
        {
        }

        // State is a sub fsm with exit pseudo states and gets a pointer to this fsm, so it can build a callback
        template <class StateType>
        std::enable_if_t<is_composite_state<StateType>::value, void> new_state_helper(
            boost::msm::back::dummy<0> = 0) const
        {
            boost::fusion::at_key<StateType>(self->m_substate_list).set_containing_sm(containing_sm);
        }
        // State is a sub fsm without exit pseudo states and does not get a callback to this fsm or state is a normal
        // state and needs nothing except creation
        template <class StateType>
        std::enable_if_t<not is_composite_state<StateType>::value and not is_pseudo_exit<StateType>::type::value, void>
        ...
        template <class StateType>
        std::enable_if_t<is_pseudo_exit<StateType>::type::value, void> new_state_helper(
            boost::msm::back::dummy<2> = 0) const
        {
            execute_return (ContainingSM::*pf)(typename StateType::event const& evt) = &ContainingSM::process_event;
            std::function<execute_return(typename StateType::event const&)> fct =
                std::bind(pf, containing_sm, std::placeholders::_1);
        ...
        {
            this->new_state_helper<State>(), create_state_helper<State>::set_sm(self);
            // create a new state with the defined id and type
            BOOST_STATIC_CONSTANT(int, state_id = (get_state_id<stt, State>::value));
            // create a visitor callback
            visitor_helper(state_id,
                boost::fusion::at_key<State>(self->m_substate_list),
    ...
    private:
        ...
        {
            visitor_args<StateType>::template helper<StateType>(self, id, astate);
        }
        template <class StateType>
    ...
    struct copy_helper
    {
        ...
        template <class StateType>
        std::enable_if_t<has_accept_sig<StateType>::type::value, void> visitor_helper(int id) const
        {
            visitor_args<StateType>::template helper<StateType>(
                m_sm, id, boost::fusion::at_key<StateType>(m_sm->m_substate_list));
        }
        template <class StateType>
        std::enable_if_t<not has_accept_sig<StateType>::type::value, void> visitor_helper(int) const
        {
            // nothing to do
        }
        library_sm* m_sm;
    ...
    void do_copy(library_sm const& rhs, boost::msm::back::dummy<0> = 0)
    {
    ...
    // helper used to call the correct entry/exit method unfortunately in O(number of states in the sub-sm) but should
    // be better than a virtual call
    ...
    struct entry_exit_helper
    {
        ...
        std::enable_if_t<IsEntry::value, void> helper(boost::msm::back::dummy<0> = 0)
        {
        ...
        template <class IsEntry, class State>
        std::enable_if_t<not IsEntry::type::value, void> helper(boost::msm::back::dummy<1> = 0)
        {
            BOOST_STATIC_CONSTANT(int, id = (get_state_id<stt, State>::value));
            if (id == state_id)
            {
    ...
    struct find_region_id
    {
        ...
        struct In<-1, Dummy>
        {
            typedef typename build_orthogonal_regions<library_sm, initial_states>::type all_regions;
            enum
            {
                region_index = find_region_index<all_regions, StateType>::value
            };
        };
        enum
        {
    ...
    struct direct_event_start_helper
    {
        ...
        std::enable_if_t<not has_direct_entry<EventType>::type::value, void> operator()(
            EventType const& evt, FsmType& fsm, boost::msm::back::dummy<0> = 0)
        {
        ...
        operator()(EventType const& evt, FsmType& fsm, boost::msm::back::dummy<1> = 0)
        {
            ...
            BOOST_STATIC_ASSERT(
                find_region_id<typename EventType::active_state::wrapped_entry>::region_index < nr_regions::value);
        ...
        {
            (static_cast<Derived*>(self))->on_entry(evt, fsm);
            boost::mpl::for_each<typename EventType::active_state, boost::msm::wrap<boost::mpl::placeholders::_1>>(
        ...
        template <class EventType, class FsmType>
        std::enable_if_t<is_pseudo_entry<typename EventType::active_state>::type::value, void> operator()(
            EventType const& evt, FsmType& fsm, boost::msm::back::dummy<3> = 0)
        {
            // entry on the FSM
            (static_cast<Derived*>(self))->on_entry(evt, fsm);
            int state_id = get_state_id<stt, typename EventType::active_state::wrapped_entry>::value;
            ...
            self->m_states[find_region_id<typename EventType::active_state::wrapped_entry>::region_index] = state_id;
            self->internal_start(evt.m_event);
            // and we process the transition in the zone of the newly active state (entry pseudo states are, according
            // to UML, a state connecting 1 transition outside to 1 inside
    ...
    private:
        ...
        struct fork_helper
        {
            ...
            void operator()(boost::msm::wrap<StateType> const&)
            {
                ...
                BOOST_STATIC_ASSERT(
                    find_region_id<typename StateType::wrapped_entry>::region_index < nr_regions::value);
    ...
    struct region_entry_exit_helper
    {
        ...
        {
            self_->m_states[region_id::value] = self_->m_history.history_entry(incomingEvent)[region_id::value];
...
public:
    ...
    // called for completion events. Default address set in the dispatch_table at init prevents no-transition detection
...
private:
    ...
    template <class StateType>
    void process_message_queue(StateType*, std::enable_if_t<not is_no_message_queue<StateType>::type::value, void>* = 0)
    {
    ...
    template <class StateType>
    void process_message_queue(StateType*, std::enable_if_t<is_no_message_queue<StateType>::type::value, void>* = 0)
    {
        // nothing to process
    }
    // helper function. In cases where the event is wrapped (target is a direct entry states) we want to send only the
    // real event to on_entry, not the wrapper.
    template <class EventType>
    static std::enable_if_t<has_direct_entry<EventType>::type::value, typename EventType::contained_event const&>
    remove_direct_entry_event_wrapper(EventType const& evt, boost::msm::back::dummy<0> = 0)
    {
        return evt.m_event;
    }
    template <class EventType>
    static std::enable_if_t<not has_direct_entry<EventType>::type::value, EventType const&>
    remove_direct_entry_event_wrapper(EventType const& evt, boost::msm::back::dummy<1> = 0)
    {
        // identity. No wrapper
        return evt;
    }
    // calls the entry/exit or on_entry/on_exit depending on the state type (avoids calling virtually) variant for FSMs
    template <class StateType, class EventType, class FsmType>
    static std::enable_if_t<is_composite_state<StateType>::value, void> execute_entry(
    ...
    // variant for states
    template <class StateType, class EventType, class FsmType>
    static std::enable_if_t<not(is_composite_state<StateType>::value or is_pseudo_exit<StateType>::type::value), void>
    ...
    template <class StateType, class EventType, class FsmType>
    static std::enable_if_t<is_pseudo_exit<StateType>::type::value, void> execute_entry(
        StateType& astate, EventType const& evt, FsmType& fsm, boost::msm::back::dummy<2> = 0)
    {
    ...
    template <class StateType, class EventType, class FsmType>
    static std::enable_if_t<is_composite_state<StateType>::value, void> execute_exit(
        StateType& astate, EventType const& evt, FsmType& fsm, boost::msm::back::dummy<0> = 0)
    {
        astate.do_exit(evt, fsm);
    }
    template <class StateType, class EventType, class FsmType>
    static std::enable_if_t<not is_composite_state<StateType>::value, void> execute_exit(
        StateType& astate, EventType const& evt, FsmType& fsm, boost::msm::back::dummy<1> = 0)
    {
    ...
    static std::enable_if_t<not(has_explicit_entry_state<TargetType>::type::value or
                                boost::mpl::is_sequence<TargetType>::type::value),
        void>
...
private:
    ...
    template <class StateType>
    struct msg_queue_helper<StateType, std::enable_if_t<is_no_message_queue<StateType>::type::value>>
    {
    };
